...
class Translator(object):
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = pred.replace('[unused5]', '').replace('[unused3]', '').replace('[PAD]', '').replace('[unused1]', '').replace(r' +', ' ').replace(' [unused2] ', '<q>').replace('[unused2]', '').strip()
                    gold_str = gold.strip()
                    if(self.args.recall_eval):
                        _pred_str = ''
                        gap = 1e3
    ...
    def _fast_translate_batch(self,
        ...
        for step in range(max_length):
            ...
            topk_beam_index = topk_ids.true_divide(vocab_size).long()
